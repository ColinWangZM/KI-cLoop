...
B_SYS, E_SYS = "<<SYS>>\n", "\n<</SYS>>\n\n"
BASE_QWEN_MODEL_PATH = os.environ.get(
    "BASE_QWEN_MODEL_PATH",
    config_data.get("BASE_QWEN_MODEL_PATH", config_data["LLAMA_MODEL_NAME"]),
)
OR_LORA_PATH = os.environ.get(
    "KGRAG_OR_LORA_PATH",
    config_data.get("OR_LORA_PATH", "checkpoints/or_lora"),
)


def cuda_available_for_generation():
    return torch.cuda.is_available() and torch.cuda.device_count() > 0


def require_generation_device():
    if cuda_available_for_generation():
        return "cuda"
    if os.environ.get("KGRAG_ALLOW_CPU", "0") == "1":
        return "cpu"
    raise RuntimeError(
        "No CUDA GPU is visible to PyTorch. `nvidia-smi` is currently failing "
        "or `/dev/nvidia*` devices are not exposed. Fix the NVIDIA driver/device "
        "visibility first, or set `KGRAG_ALLOW_CPU=1` to force a very slow CPU run."
    )


def model_load_kwargs(branch_name, cache_dir):
    device = require_generation_device()
    if device == "cuda":
        return {
            "device_map": "auto",
...
def pipeline_kwargs(max_new_tokens, stream=False, tokenizer=None):
    device = require_generation_device()
    kwargs = {
    ...
    if device == "cuda":
        ...
        kwargs["device"] = -1
    if stream:
        kwargs["streamer"] = TextStreamer(tokenizer)
    return kwargs
...
def llama_model(model_name, branch_name, cache_dir, temperature=0, top_p=1, max_new_tokens=512, stream=False, method='method-1'):
    if method == 'method-1':
        # 使用正确的模型和 LoRA 路径
        base_model_name = BASE_QWEN_MODEL_PATH
    ...
    if not stream:
        pipe = pipeline("text-generation",
                    model = model,
                    tokenizer = tokenizer,
                    **pipeline_kwargs(max_new_tokens, stream=False)
                    )
    else:
        pipe = pipeline("text-generation",
                    model = model,
                    tokenizer = tokenizer,
                    **pipeline_kwargs(max_new_tokens, stream=True, tokenizer=tokenizer)
                    )        
...
def retrieve_context(question, vectorstore, embedding_function, node_context_df, context_volume, context_sim_threshold, context_sim_min_threshold, edge_evidence, api=False):
    entities = []
    ...
    if entities:
    ...
    else:#不提取实体（实体提取注释掉了，直接对问题进行相似性检索）
        node_hits = vectorstore.similarity_search_with_score(question, k=1)
        ...
        for node in node_hits:
            ...
            if not api:
                ...
                #print("node_context:(notapi)",node_context)
            else:
                node_context, context_table = get_context_using_spoke_api(node_name)


                #查看context_table和node_context
                # print("context_table:",context_table)
            ...
            # ... existing code ...
            #print("\n相似度排序后的值:")
            for idx, (sim_value, _) in enumerate(similarities[:5]):  # 只打印前5个相似度值
                # sim_value是一个numpy数组，需要转换为float
                similarity = float(sim_value)
                #print(f"第{idx+1}个相似度值: {similarity:.4f}")
                        
            percentile_threshold = np.percentile([float(s[0]) for s in similarities], context_sim_threshold)
            #print(f"\n百分位数阈值 (percentile_threshold): {percentile_threshold:.4f}")
            #print(f"最小相似度阈值 (context_sim_min_threshold): {context_sim_min_threshold:.4f}")
                        
            high_similarity_indices = [s[1] for s in similarities if s[0] >= percentile_threshold and s[0] > context_sim_min_threshold]
            #print(f"通过阈值筛选后的索引数量: {len(high_similarity_indices)}")
